...
(function () {
  ...
  const DISCOUNT = { monthly: 0, half: 0.05, yearly: 0.1 }
  ...
  function renderTierCards () {
    document.querySelectorAll('[data-tier-price]').forEach(el => {
      ...
      el.textContent = BASE_PRICE[tier]
        ? fmt(priceFor(BASE_PRICE[tier]))
        : el.textContent
    })
    document.querySelectorAll('[data-billed-note]').forEach(el => {
      if (dict && dict.pricing && dict.pricing.billing) {
        el.textContent = dict.pricing.billing[BILLED_KEY[billing]]
      }
    })
    document.querySelectorAll('.billing-toggle button').forEach(btn => {
      ...
      btn.setAttribute(
        'aria-pressed',
        btn.getAttribute('data-billing') === billing ? 'true' : 'false'
      )
    })
  }
